...
import { Users, Calendar, Heart, Plane } from "lucide-react";
  ...
  >
    <div className="luxury-section__header">
      ...
      <p className="luxury-section__lead">
        ...
        provide everything you need to immerse and refine. Come as you are. Leave changed. Book an
        Airport Run so you land well, walk the land on a tour, gather your people for a retreat or
        workshop we host and plan, or begin with healing, including our six-week Realignment
        Program. Small groups. Real roots.
      </p>
    </div>

    <div className="luxury-approach-grid luxury-paths-grid">
      {offerings.map(
        ({ icon: Icon, eyebrow, title, text, href, cta, secondaryHref, secondaryCta, featured }) => (
          <div
            key={title}
            className={`luxury-approach-card luxury-path-card${featured ? " luxury-path-card--featured" : ""}`}
          >
            {featured ? <span className="luxury-path-card__featured-tag">Featured</span> : null}
