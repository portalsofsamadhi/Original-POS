...
function testPaymentConfig() {
  ...
  try {
    // Simulate payment config check
  const paypalConfigured = !!(process.env.VITE_PAYPAL_CLIENT_ID || 'REDACTED_PAYPAL_CLIENT_ID');
  const stripeConfigured = !!(process.env.VITE_STRIPE_PUBLIC_KEY || 'REDACTED_STRIPE_PUBLIC_KEY');
    
    console.log('✅ PayPal Configuration:', paypalConfigured ? 'READY' : 'MISSING');
    console.log('✅ Stripe Configuration:', stripeConfigured ? 'READY' : 'MISSING');
...
function testEmailConfig() {
  ...
  try {
  const smtpConfigured = !!(process.env.SMTP_USER || 'REDACTED_SMTP_USER');
  const smtpPassword = !!(process.env.SMTP_PASS || 'REDACTED_SMTP_PASS');
    
    console.log('✅ SMTP Host: smtp.gmail.com');
    console.log('✅ SMTP Port: 587');
    console.log('✅ SMTP User:', smtpConfigured ? 'CONFIGURED' : 'MISSING');
