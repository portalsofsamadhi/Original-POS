...
function Test-Security {
    ...
    if ($response) {
        ...
        if ($content -match 'pk_live_51QzB52K61YEYHkwd1FCWGRFEt') { 
            $exposedSecrets += "Stripe public key" 
        }
        if ($content -match 'AcChyGp-9gAhHh_') { 
            $exposedSecrets += "PayPal client ID" 
        }
        if ($content -match 'xofw jkau') { 
            $exposedSecrets += "SMTP password" 
        }
        ...
        if ($exposedSecrets.Count -eq 0) {
            ...
            Write-Host " Fail - Found: $($exposedSecrets -join ', ')" -ForegroundColor Red
            $script:failed++
        }
    }
